...
const sadece = (role_name) => (req, res, next) => {
  ...
  */
  ...
  // let isAuth = req.decoded.rol_name === role_name;
  // if (isAuth) {
  //   next();
  // } else res.status(403).json({ message: "Bu, senin için değil" });
};
...
const usernameVarmi = async (req, res, next) => {
  ...
  */
  try {
    let userByUsername = await userModel.goreBul({
    ...
    const isExist =
      ...
      bcryptjs.compareSync(req.body.password, userByUsername[0].password);
    if (!isExist) {
